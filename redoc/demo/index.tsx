...
const demos = win.demos || [
  { value: 'http://127.0.0.1:8000/docs/api-docs-backend.json', label: 'Instagram' },
  { value: 'http://127.0.0.1:8000/docs/api-docs.json', label: 'Instagram' },
];

const DEFAULT_SPEC = '/docs/api-docs.json';
...
> {
  ...
  onLoaded = () => {
    ...
    const win: any = window;
    const callback = win.callback;
    callback(specUrl);
  };

  handleChange = (url: string) => {
    window.location.href = window.location.origin + window.location.pathname + '?url=' + url;
  };
  ...
  render() {
    ...
    return (
      <>
        <Heading>
          <a href=".">
            <Logo src="https://center.chebaba.com/img/default2020.jpg" />
          </a>
          <ControlsContainer>
            <ComboBox
        ...
        <RedocStandalone specUrl={proxiedUrl} options={{ scrollYOffset: 'nav', expandDefaultServerVariables: true}} onLoaded={this.onLoaded} />
      </>
    );
  }
}
